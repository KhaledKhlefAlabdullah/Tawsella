...
<main id="main" class="main">
    <div class="col-12">
        <div class="card recent-sales overflow-auto">
            ...
            <div class="card-body">
                <h5 class="card-title">{{ __('الطلبات المكتملة') }} <span>| {{ __('سجل') }}</span></h5>
                    ...
                    <table class="table table-borderless datatable" data-toggle="table" data-pagination="true"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th data-sortable="true">{{ __('التكلفة') }}</th>
                                <th data-sortable="true">{{ __('التاريخ') }}</th>
                                <th>{{ __('موقع النهاية') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ date('Y-m-d', strtotime($movement->date)) }}</td>
                                <td>
                                    @if ($movement->taxi_id)
                                    <a href="{{ route('movement.completed.map', ['taxiMovement' => $movement]) }}"
                                        class="btn btn-success">{{ __('موقع العميل') }}</a>
                                    @else
